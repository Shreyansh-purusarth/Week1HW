...
You'll be modifying the file `src/Week1/process_image.c`. We've also included a python compatability library. `uwimg.py` includes the code to access your C library from python. `tryit.py` has some example code you can run. We will build the library using `make`. Simply run the command:
...
Your first task is to fill out these two functions in `src/Week1/process_image.c`:
...
Fill in the function `image copy_image(image im)` in `src/Week1/process_image.c` with your code.
...
If you have completed the homework then upload all the images in a separate folder named **WEEK1**
and upload it on your google drive and send it in.